...
public class Main {
    ...
    public static void main(String[] args) {
        VcsUtils.println("Iveskite DB pavadinima");
        String userDb = VcsUtils.inputWord();
        Connection conn = null;
        try {
            conn = VcsDbUtils.connect(userDb);
            Statement state = conn.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
            ResultSet result = state.executeQuery("select * from person where vardas='Justinas'");
            while (result.next()){
            ...
            result.updateString("vardas", "Justinas");
            result.insertRow();
            ...
            try {
                ...
                VcsUtils.println("neuzsidare resursas");
            }
